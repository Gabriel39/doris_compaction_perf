        ...
        if data_gen:
            ...
            with open(file_name, 'w') as f:
                for i in range(j * 500000, j * 500000 + 500000):
                    item = {'user_id': i % group_size if group_size > 0 else i, 'username': 'user%s' % (i % group_size if group_size > 0 else i),
                            'city': 'Beijing', 'age': 18, 'sex': i % 2, 'phone': '2020-10-0%s' % (i % group_size if group_size > 0 else i),
                            'address': 'Beijing', 'register_time': '2020-01-01 10:00:00'}
                    arr.append(item)
                json.dump(arr, f)
