        ...
        if data_gen:
            ...
            with open(file_name, 'w') as f:
                for i in range(j * 500000, j * 500000 + 500000):
                    item = {'user_id': i % group_size if group_size > 0 else i, 'date': '2017-10-0%s' % (i % 9 + 1),
                            'city': 'Beijing', 'age': 18, 'sex': i % 2, 'last_visit_date': '2020-10-0%s' % (i % 9 + 1),
                            'cost': random.randint(1, 100), 'max_dwell_time': random.randint(10, 100),
                            'min_dwell_time': random.randint(1, 10)}
                    arr.append(item)
                json.dump(arr, f)
